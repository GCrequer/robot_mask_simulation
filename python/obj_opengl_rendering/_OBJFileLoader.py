    ...
    def __init__(self, filename, swapyz=False):
        ...
        for line in open(filename, "r"):
            ...
            if values[0] == 'v':
                ...
                # Calculate face normal and assign to all vertices in the face
                face_normal = np.cross(
                    np.array(self.vertices[face[1] - 1]) - np.array(self.vertices[face[0] - 1]),
                    np.array(self.vertices[face[2] - 1]) - np.array(self.vertices[face[0] - 1])
                )

                face_normal /= np.linalg.norm(face_normal)
                for i in range(len(face)):
                    norms[i] = len(self.normals) + 1
                    self.normals.append(face_normal.tolist())
...
#TODO : adapt the following code for urdf generation
    def generate(self):
        ...
        for face in self.faces:
            ...
            glColor([0., 0. , 0.])  # default to white if Kd is not present
